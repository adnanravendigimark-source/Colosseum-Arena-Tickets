...
export default function BlogIndexSidebar({
  ...
  searchQuery,
  onSearchChange,
  ctaHeading = "Book Your Colosseum Tickets",
  ctaBody = "Best prices, secure booking and instant confirmation.",
  ctaButtonText = "Compare Tickets →",
}: {
  posts: Post[];
  ...
  return (
    <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
      {/* Search Widget */}
      <div className="flex rounded-xl border border-warmstone-300 bg-cream-50 overflow-hidden shadow-sm focus-within:border-olive-700">
        <input
          type="text"
          value={searchQuery || ""}
      ...
      {categories.length > 0 && (
        <div className="rounded-2xl border border-warmstone-200 bg-cream-50 p-5 shadow-sm">
          <p className="font-display text-base font-bold text-charcoal-900">Categories</p>
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-lg px-2.5 py-2 text-xs font-semibold transition ${
                    isSelected
                      ? "bg-warmstone-200 text-olive-800 font-bold"
                      : "text-charcoal-700 hover:bg-warmstone-100 hover:text-olive-700"
                  }`}
                >
                  <span>{cat.name}</span>
                  <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-warmstone-200/90 px-1.5 text-[10px] font-bold text-charcoal-700">
                    {cat.count}
                  </span>
                </button>
              );
      ...
      {popular.length > 0 && (
        <div className="rounded-2xl border border-warmstone-200 bg-cream-50 p-5 shadow-sm">
          <p className="font-display text-base font-bold text-charcoal-900">Popular Articles</p>
      ...
      {/* Book Your Colosseum Tickets Promo Card */}
      <div className="relative overflow-hidden rounded-2xl border border-warmstone-300 bg-[#ECE6D8] p-6 text-center shadow-sm">
        ...
        <p className="mt-3.5 font-display text-base font-bold text-charcoal-900">{ctaHeading}</p>
        <p className="mt-1.5 text-xs leading-relaxed text-charcoal-600">{ctaBody}</p>
        <a
          href="/#tours"
