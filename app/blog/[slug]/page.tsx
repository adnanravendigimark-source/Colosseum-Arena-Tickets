...
export default async function Post({ params }: { params: { slug: string } }) {
  ...
  const articleJsonLd = buildArticleJsonLd({
    ...
    authorName: "Colosseum Arena Tickets",
    ...
    <>
      ...
      <main>
        <div className="mx-auto max-w-4xl px-4 pt-8 sm:px-6">
          <Link href="/blog" className="inline-flex items-center gap-1 text-sm font-bold text-olive-700 hover:underline">
            {s.backToGuidesText}
          </Link>
          <div className="mt-6 flex items-center gap-3 text-xs font-bold uppercase tracking-wider text-olive-800">
            <span className="rounded-md bg-olive-100 px-2.5 py-1 text-olive-800">{post.category}</span>
            <span className="h-1 w-1 rounded-full bg-warmstone-300" />
            <span className="text-sage-600 font-medium">{post.readTime}</span>
          </div>
          <h1 className="mt-3 font-display text-3xl font-bold leading-tight text-charcoal-800 sm:text-5xl">
            {post.title}
          </h1>
          {post.excerpt && <p className="mt-4 max-w-3xl text-lg leading-relaxed text-charcoal-600">{post.excerpt}</p>}
          <div className="relative mt-8 aspect-[21/9] w-full overflow-hidden rounded-2xl border border-warmstone-200 shadow-md">
            <SafeImage
              src={post.image}
              alt={post.imageAlt}
              fill
        ...
        <div className="mx-auto max-w-6xl px-4 pb-20 pt-12 sm:px-6 lg:grid lg:grid-cols-[1fr_21rem] lg:gap-12">
          <div>
            ...
            <div className="mt-12 rounded-2xl border border-warmstone-300 bg-gradient-to-br from-warmstone-100/60 via-cream-50 to-warmstone-100/40 p-8 shadow-sm">
              <p className="font-display text-xl font-bold text-charcoal-800">{post.ctaHeading}</p>
              <p className="mt-2 text-sm leading-relaxed text-charcoal-600">{post.ctaBody}</p>
              <Link
                href={post.ctaButtonHref}
                className="mt-6 inline-flex rounded-xl bg-olive-700 px-6 py-3 text-sm font-bold text-cream-100 shadow-md ring-1 ring-sage-400/30 transition hover:bg-olive-800 hover:scale-[1.02]"
              >
                {post.ctaButtonText}
              </Link>
            </div>
          ...
          <div className="mt-14 lg:mt-0 lg:border-l lg:border-warmstone-200 lg:pl-10">
