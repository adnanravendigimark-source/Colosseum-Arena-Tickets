...
const DEFAULT_PRIVACY_POLICY: PrivacyPolicy = {
  ...
  noFollow: false,
  canonicalUrl: "",
  metaTitle: "Privacy Policy | Colosseum Arena Tickets Rome",
  metaDescription: "Privacy Policy for Colosseum Arena Tickets — how we protect your information when exploring Rome ticket guides.",
  ogTitle: "",
  ogDescription: "",
  ogImage: "",
};
...
export async function savePrivacyPolicy(data: PrivacyPolicy): Promise<void> {
  await sql`
    INSERT INTO privacy_policy (
      id, title, last_updated, last_updated_label, empty_state_text, content,
      no_index, no_follow, canonical_url, meta_title, meta_description,
      og_title, og_description, og_image
    ) VALUES (
      1, ${data.title}, ${data.lastUpdated}, ${data.lastUpdatedLabel}, ${data.emptyStateText},
      ${JSON.stringify(data.content || [])}::jsonb,
      ${!!data.noIndex}, ${!!data.noFollow}, ${data.canonicalUrl || ""},
